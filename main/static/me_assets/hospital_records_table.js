...
const post_hospital_records = async (payload) => {
	...
	const data = await fetch("/manage_hospital_records/", config).then((response) =>
		response.json()
	);
	console.log(data, data.status)
	if (data.status == 1){
		$('.hospital_record_table_rows').prepend(construct_hospital_record_table_row(data.data));
	}
...
const delete_hospital_record = async (payload) => {
	...
	const data = await fetch("/manage_hospital_records/?delete=1", config).then((response) =>
		response.json()
	);
	console.log(data)
	if (data.status !== 0){
...
const construct_hospital_record_table_row = (
	data
) => `
